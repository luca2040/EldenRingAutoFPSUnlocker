/*
PatternScan.cs from https://github.com/uberhalit/EldenRingFpsUnlockAndMore
Original license: https://github.com/uberhalit/EldenRingFpsUnlockAndMore/blob/main/LICENSE
*/
...
namespace EldenRingAutoFPSUnlocker.utils
{
    ...
    internal class PatternScan : IDisposable
    {
        ...
        internal PatternScan(IntPtr hProcess, ProcessModule pModule)
        {
            ...
            if (!DllAPI.ReadProcessMemory(hProcess, dwStart, bData, (ulong)nSize, out IntPtr lpNumberOfBytesRead))
            {
                LogFile.Log("Could not read memory in PatternScan()!");
                return;
            }
            if (lpNumberOfBytesRead.ToInt64() != nSize || bData == null || bData.Length == 0)
            {
                LogFile.Log("ReadProcessMemory error in PatternScan()!");
                return;
            }
        }
